...
\begin{enumerate}
  ...
  \item \textbf{Number of installations}: ``number of installations made'' is an attribute of Team.
        ...
        This is a voulontary redundancy, consequence of a frequent operation. 
\end{enumerate}
...
\begin{figure}[H]
  ...
  \caption{Skeleton Schema -- Main entities and relationships (attributes omitted)}
\end{figure}
...
\begin{table}[H]
  ...
  LOCATED_IN & An event location hosts one or more bookings; each booking is at one location
...
\begin{table}[H]
  ...
  LOCATED_IN      & R & 100{,}000  & 1:1 cardinality with booking \\ \hline
...
\begin{table}[H]
  ...
  \hline \textbf{Concept} & \textbf{Type} & \textbf{Accesses} & \textbf{R/W} \\ \hline
  Booking         & E & 1 & W \\ \hline
  Team            & E & 1 & R (FK lookup) \\ \hline
  Customer        & E & 1 & R (FK lookup) \\ \hline
  Event\_Location & E & 1 & R (FK lookup) \\ \hline
  Team            & E & 1 & W (Installation\_Number + 1) \\ \hline
  Event\_Location & E & 1 & W (Booking\_Count + 1) \\ \hline
  \end{tabularx}
  \caption{Operation 2 -- Total: $(2+1+1+1+2+2) \times 300 = 2{,}700$ ops/day}
\end{table}
...
\begin{table}[H]
  ...
  \hline \textbf{Concept} & \textbf{Type} & \textbf{Accesses} & \textbf{R/W} \\ \hline
  Customer        & E & 1 & R (FK lookup) \\ \hline
  Event\_Location & E & 1 & W \\ \hline
  \end{tabularx}
  \caption{Operation 3 -- Total: $(1+2) \times 50 = 150$ ops/day}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|X|X|X|X|}
  \hline \textbf{Concept} & \textbf{Type} & \textbf{Accesses} & \textbf{R/W} \\ \hline
  Event\_Location & E & 1   & R (find the location) \\ \hline
  Booking         & E & 100 & R (avg bookings per location: 100{,}000/1{,}000) \\ \hline
  Team            & E & 100 & R (DEREF per booking to get team info) \\ \hline
  \end{tabularx}
  \caption{Operation 4 -- Total: $(1+100+100) \times 20 = 4{,}020$ ops/day}
\end{table}
...
Without the redundancy, Op5 requires joining \texttt{Event\_Location} and \texttt{Booking}:
\begin{table}[H]
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{|X|X|X|X|}
  \hline \textbf{Concept} & \textbf{Type} & \textbf{Accesses} & \textbf{R/W} \\ \hline
...
\end{table}
\noindent Cost Op5 without redundancy: $(1{,}000 + 100{,}000) \times 5 = 505{,}000$ ops/day.
...
\]
\[
  \text{Total \textbf{without} redundancy} = 20 + 1{,}500 + 150 + 4{,}020 + 505{,}000
  = \mathbf{510{,}690} \text{ ops/day}
\]
\[
  \text{Total \textbf{with} redundancy} = 20 + 2{,}700 + 150 + 4{,}020 + 5{,}000
  = \mathbf{11{,}890} \text{ ops/day}
\]

\noindent \textbf{Conclusion}: keeping \texttt{Booking\_Count} reduces the total workload by a factor
of $\sim$43. The redundancy is justified and maintained automatically by a trigger.

\subsubsection{\texttt{Installation\_Number} on Team}

Without the redundancy, computing the installation count for a team requires counting all bookings
handled by that team:
\[
  \text{Bookings per team on average} = 100{,}000 / 100 = 1{,}000 \text{ reads per query}
\]
The break-even (assuming this is queried $q$ times per day):
\[
  q \times 1{,}000 > 300 \times 2 \quad \Rightarrow \quad q > 0.6
\]
Since $q \geq 1$, keeping the redundancy is justified.
...
\begin{table}[H]
  ...
  \texttt{Team(\underline{Code}, Name, Max\_Members, Installation\_Number*, Depot\_ID)} \\ \hline
  \texttt{Member(\underline{ID}, Name, Surname, Phone, Email, Team\_Code)} \\ \hline
  \texttt{CentralOffice(\underline{ID}, Name, Address, City, Province, Number\_Of\_Employees)} \\ \hline
  \texttt{Depot(\underline{ID}, Name, Address, City, Province, Number\_Of\_Employees, Region)} \\ \hline
  \texttt{Municipality(\underline{ID}, Name, Depot\_ID)} \\ \hline
  \end{tabularx}
  \caption{Relational Schema (* = redundant derived attribute)}
\end{table}

\noindent\textbf{Translation rules applied:}
\begin{enumerate}
  \item \textbf{Generalisation Office}: two separate tables \texttt{CentralOffice} and \texttt{Depot}
        sharing the same attribute structure; \texttt{Depot} adds \texttt{Region}.
  \item \textbf{COVERS (Depot $\to$ Municipality, 1:N)}: FK \texttt{Depot\_ID} added to
        \texttt{Municipality}.
  \item \textbf{BELONGS\_TO (Team $\to$ Depot, N:1)}: FK \texttt{Depot\_ID} added to \texttt{Team}.
  \item \textbf{HAS (Team $\to$ Member, 1:N)}: FK \texttt{Team\_Code} added to \texttt{Member}.
  \item \textbf{PLACE (Customer $\to$ Event\_Location, 1:N)}: FK \texttt{Customer\_ID} added to
        \texttt{Event\_Location}.
  \item \textbf{RESERVATION (Customer $\to$ Booking, 1:N)}: FK \texttt{Customer\_ID} added to
        \texttt{Booking}.
  \item \textbf{BL (Event\_Location $\to$ Booking, 1:N)}: FK \texttt{Location\_ID} added to
        \texttt{Booking}.
  \item \textbf{HANDLED (Booking $\to$ Team, N:1)}: FK \texttt{Team\_Code} added to \texttt{Booking}.
\end{enumerate}
...
\noindent\textbf{Note on diagram vs.\ final implementation.}
The UML diagram above was produced during the analysis phase and reflects the
N:M interpretation of the BELONG relationship (Team $\leftrightarrow$ Depot),
which included a \texttt{Team\_Depot} join table.
Based on the requirements text (``Depots have one or more setup teams''), this
was subsequently resolved to a \textbf{1:N} relationship (Ambiguity Resolution \#3):
the join table was removed and a direct \texttt{Depot REF DepotTY} attribute was
added to \texttt{TeamTY}.
Additionally, the diagram pre-dates two design decisions made in the logical phase:
(i) the \textit{Office} generalisation (\texttt{OfficeTY}/\texttt{CentralOfficeTY}), and
(ii) the redundant \texttt{Booking\_Count} attribute on \texttt{Event\_LocationTY}.
The Oracle implementation in Section~\ref{sec:implementation} reflects all final decisions.
...
Inter-entity references use \texttt{REF} attributes.

The generalisation \textit{Office} $\to$ \{\textit{CentralOffice}, \textit{Depot}\} is realised with
Oracle's \textbf{type inheritance}: \texttt{OfficeTY} is declared \texttt{NOT FINAL}; \texttt{DepotTY}
extends it with \texttt{UNDER} and adds the \texttt{Region} attribute.

\subsection{Object Types}

\begin{lstlisting}
-- Supertype: shared attributes of CentralOffice and Depot
CREATE OR REPLACE TYPE OfficeTY AS OBJECT (
    ID                  VARCHAR2(20),
    Name                VARCHAR2(100),
...
-- Depot specialisation: adds Region
CREATE OR REPLACE TYPE DepotTY UNDER OfficeTY (
    Region VARCHAR2(100)
);
...
\begin{lstlisting}
-- CentralOffice: OF supertype OfficeTY (no extra attributes)
CREATE TABLE CentralOffice OF OfficeTY (
    ID   PRIMARY KEY,
    Name NOT NULL,
...
Before each member insert, the trigger reads the team's declared maximum and the current count of its
members. If the limit is already met, the insert is aborted (BR9).
...
Triggers 2 and 3 both fire \texttt{AFTER INSERT ON Booking}. Each updates one redundant counter,
keeping \texttt{Installation\_Number} and \texttt{Booking\_Count} always consistent without any
